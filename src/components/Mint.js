...
const Mint = () => {
    ...
    const connect = async () => {
        ...
        if (networkId.toString() === '10000') {
            let provider = new ethers.providers.Web3Provider(window.ethereum, "any");
            let tmpContract = new ethers.Contract('0xbad64f85727D27e67631BC1a5B6e339cd1830c4a', ABI, provider.getSigner());
            setContract(tmpContract);
            setConnected(true);

            try {
                setAlreadyMinted((await tmpContract.hasMinted(account)).toNumber());
            } catch(e) {
            ...
            setError('Please connect to SmartBCH network.')
        }
    };

    const add = async () => {
        setMintAmount(Math.min(mintAmount + 1, 5 - (alreadyMinted)));
    }
            ...
            {connected !== false ? (
                <div style={{textAlign: "center"}}>
                    ...
                    {alreadyMinted < 5 ? (<>
                        ...
                        <div>You received 5 NFTs, yay!</div>
                    </>)}
                </div>
            ) : (
                <div>
